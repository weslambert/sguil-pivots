...
proc GetLookupIP { arg url } {
	...
	# Kibana and Kibana links are relative links, like this:
	# /elsa
	# If $url is relative, we need to prefix it with https:// and the IP or hostname of the server
	if {[string first "http" $url] == -1} {
		set prefix "https://$SERVERHOSTSELECTED"
		set url [ join [list $prefix $url] "" ]
	}
	set url [string map "REPLACE_WITH_IP_ADDRESS $ipAddr" $url]
...
proc GenCaseHive {} {
	...
	if { [catch { exec /bin/sh -c "python genCaseHive.py -t '$eventMsg' -d '$sensor | $date | $srcip | $srcport | $dstip | $dstport | $eventMsg' -s '1'" & } msg] } {
            tk_messageBox -message "Something seems to have gone wrong!" -type ok
	} else {
            tk_messageBox -message "A case has been generated!" -type ok
        }
    }
}
...
proc GenEventFIR {} {
    ...
    if { $ACTIVE_EVENT && !$MULTI_SELECT} {
        ...
        set dstport [$CUR_SEL_PANE(name) getcells $selectedIndex,dstport]
        set eventMsg [$CUR_SEL_PANE(name) getcells $selectedIndex,event]
        if { [catch { exec /bin/sh -c "python genEventFIR.py -j '$eventMsg' -d 'Test' -s '2' -c '3' -l '1' -p '8' -t '2' -a '3'" & } msg] } {
            tk_messageBox -message "Something seems to have gone wrong!" -type ok
        } else {
